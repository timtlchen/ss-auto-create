...
resource "google_compute_global_forwarding_rule" "ss-global-forwarding-rule" {
  name       = "${var.project_tag}-global-forwarding-rule"
...
resource "google_compute_backend_service" "ss-backend" {
  ...
  backend {
    group = "${google_compute_instance_group.ss-instance-group.self_link}"
  }
